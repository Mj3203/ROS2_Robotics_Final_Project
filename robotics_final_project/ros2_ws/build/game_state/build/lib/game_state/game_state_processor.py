import rclpy
from rclpy.node import Node
from std_msgs.msg import Image, String # ROS2 message types
from cv_bridge import CvBridge # Bridge to convert between ROS and OpenCV images
import cv2 # OpenCV for image processing
import numpy as np

from ultralytics import YOLO # YOLO model for object detection

from rcply.client import Client # ROS2 action server client
from game_state.srv import GetBoardState # Action definition

import json

class GameStateProcessor(Node):
    def __init__(self):
        super().__init__('game_state_processor')

        # Initialize CvBridge
        self.bridge = CvBridge()

        # Placeholder for the latest image
        self.latest_image = None

        # Create a subscriber for Image messages on the 'processed_video_feed' topic
        self.image_subscriber = self.create_subscription(Image, 'processed_video_feed', self.image_callback, 10)

        # Create a service server 
        self.srv = self.create_service(GetBoardState, "get_board_state", self.handle_get_board_state)

        self.get_logger().info('GameStateProcessor node is ready. Service server listening on "get_board_state".')

        # Load YOLO model
        self.model = YOLO('trained_models/best.pt')
    
    def image_callback(self, data):
        try:
            # Convert ROS Image message to OpenCV image
            self.latest_image = self.bridge.imgmsg_to_cv2(data, desired_encoding='bgr8')
        except Exception as e:
            self.get_logger().error(f"Error converting ROS Image to OpenCV image: {e}")
            self.latest_image = None
            return
        
    def detect_board_state(self, homographed_image):
        results = self.model(homographed_image)
        res = results[0]

        board_dict = {}

        file_index = ['a','b','c','d','e','f','g','h']
        rank_index = ['8','7','6','5','4','3','2','1']

        square_x_length = 740/8
        square_y_length = 800/8
        
        for box in res.boxes:
            cx = int(box.xywh[0][0])
            cy = int(box.xywh[0][1])

            cls = int(box.cls[0])
            name = self.model.names[cls]

            file = int(cx // square_x_length)
            rank = int(cy // square_y_length)

            fen_square = file_index[file] + rank_index[rank]
            board_dict[fen_square] = name

        return board_dict
        
    def get_board_state_callback(self, request, response):
        self.get_logger().info("Received request for board current board state.")
        
        if self.latest_image is None:
            self.get_logger().error("No image available for processing.")
            response.board_state = ""
            return response
        
        board_dict = self.detect_board_state(self.latest_image)

        response.board_state = json.dumps(board_dict)
        return response

    def destroy_node(self):
        super().destroy_node()

def main(args=None):
    rclpy.init(args=args)
    game_state_processor = GameStateProcessor()

    try:
        rclpy.spin(game_state_processor)
    except KeyboardInterrupt:
        pass
    
    # Destroy the node explicitly
    game_state_processor.destroy_node()
    rclpy.shutdown()

if __name__ == '__main__':
    main()